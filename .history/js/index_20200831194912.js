            ...
            async function init() {
                ...
                $plan.add(power => {
                        ...
                        $navBoxList.click(function() {
                                let index = $(this).index(); //获取点击a标签的索引
                                let text = $(this).html().trim();

                                if ((text == "客户管理") && !/customerall/.test(power) || (text === "组织结构") && !/(userhandle|departhandle|jobhandle))
                                    $(this).addClass("active").siblings().removeClass("active"); handGroup(index); initIndex = index;
                                });
                        })
                    //代表你登录成功了
                    ...
                    power.code === 0 ? power = power.power : null; baseInfo.code === 0 ? baseInfo = baseInfo.data : null;
